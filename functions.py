...
def train(args, gen_net: nn.Module, dis_net: nn.Module, gen_optimizer, dis_optimizer, gen_avg_param, train_loader,
          epoch, writer_dict, fixed_z,  schedulers=None, trend1 = None, trend2 = None):
    writer = writer_dict['writer']
    gen_step = 0
    # train mode
    gen_net.train()
    ...
    for iter_idx, (imgs, _) in enumerate(tqdm(train_loader)):
        ...
        # Adversarial ground truths
        real_imgs = imgs.to(args.gpu).float()

        # Sample noise as generator input
        z = torch.from_numpy(np.random.normal(0, 1, (imgs.shape[0], args.latent_dim))).to(args.gpu).float()
        ...
        real_validity = dis_net(real_imgs)
        fake_imgs = gen_net(z,trend1, trend2).detach()
        ...
        if global_steps % (args.n_critic * args.accumulated_times) == 0:
            ...
            for accumulated_idx in range(args.g_accumulated_times):
                gen_z = torch.from_numpy(np.random.normal(0, 1, (args.gen_batch_size, args.latent_dim))).to(args.gpu).float()
                gen_imgs = gen_net(gen_z,trend1,trend2)
